...
import { createNodeConnection, NODE_RETIRE_OPTIONS } from './node/connection.ts'
import { NodeStartupError, type NodeEvent, type NodeInterpreter } from './node/types.ts'
import { retireProcessGroup } from './process-group.ts'
...
async function createConnection(child: ChildProcess, onEvent: (event: NodeEvent) => void) {
  try {
    ...
    const cleanup = await retireProcessGroup(child, NODE_RETIRE_OPTIONS)
    if (cleanup.confirmed) {
      throw error
    }

    const message = errorMessage(error)
    throw new NodeStartupError(message, async () => retireProcessGroup(child, NODE_RETIRE_OPTIONS))
  }
}

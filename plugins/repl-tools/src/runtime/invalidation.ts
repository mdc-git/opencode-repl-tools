...
function invalidateCell(cell: Cell): Effect.Effect<void> {
  return Effect.gen(function* () {
    suppressCell(cell)
    const result = yield* cleanupInvalidated(cell)
    yield* warnCleanup(cell, result)
    yield* Scope.close(cell.scope, Exit.void)
  })
}

export function invalidateCells(targets: readonly Cell[]): Effect.Effect<void> {
  return Effect.forEach(targets, invalidateCell, { discard: true })
}

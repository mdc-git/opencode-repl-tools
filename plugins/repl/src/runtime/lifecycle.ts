...
function hardRetireForCancel(
  ...
  return Effect.gen(function* () {
    ...
    const cleanup = yield* Effect.promise(() => safeShutdown(interpreter))
    if (!cleanup.confirmed) {
      const message = cleanup.message ?? 'hard cancellation teardown could not be confirmed'
      yield* state.locked(() =>
        Effect.sync(() => failCancelCleanup(cell, job, interpreter, message))
      )
      return expected('lifecycle', message)
    }
    yield* state.locked(() => Effect.sync(() => finishHardCancel(cell, job, interpreter)))
...
function waitStartingCancel(cell: Cell, job: Job): Effect.Effect<JobOperationOutput> {
  return Effect.promise(() => job.completion.promise).pipe(
    Effect.map(() =>
      cell.lifecycle === 'failed'
...
export function cancelWork(
  ...
  return Effect.gen(function* () {
    ...
    yield* Effect.promise(() => interruptWithGrace(cell, job, prepared.interpreter))
...
function cleanupCell(cell: Cell): Effect.Effect<CleanupResult> {
  ...
  const interpreter = current.interpreter
  if (interpreter !== undefined) {
  ...
  return Effect.promise(() => safeRetry(retry))
}
...
export function resetCell(state: RuntimeState, cell: Cell): Effect.Effect<ResetOutput> {
  return Effect.gen(function* () {
    ...
    yield* state.locked((map) => Effect.sync(() => removeResetCell(map, cell)))
